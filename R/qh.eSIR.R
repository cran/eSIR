...
qh.eSIR <- function(Y,
  ...
  if (length(change_time) != length(phi0)) {
    stop("We need the length of vector change_time to be the length of phi0. ")
  }
  ...
  posterior <- suppressWarnings(
    jags.model(model.spec, data = list("Y" = Y, "R" = R, "T_prime" = T_prime, "phi_vec" = phi_vec, "gamma_H_vec" = gamma_H_vec), n.chains = nchain, n.adapt = nadapt)
  )
  ...
  colnames(out_table) <- c(
    ...
    "thetaQ_last_obs_p_mean",
    "thetaQ_last_obs_p_ci_low",
    "thetaQ_last_obs_p_ci_med",
    "thetaQ_last_obs_p_ci_up",
    "R0_p_mean",
    "R0_p_ci_low",
    "R0_p_ci_med",
    "R0_p_ci_up",
  ...
  if (save_mcmc) {
    save(theta_pp, thetaQ_pp, Y_pp, R_pp, file = paste0(file_add, casename, "_forecast_MCMC.RData")) # @
...
if (FALSE) {
  ...
  res.q <- qh.eSIR(Y, R,
                   begin_str = "01/13/2020", death_in_R = 0.4,
                   phi0 = phi0, change_time = change_time,
                   casename = "Hubei_q", save_files = FALSE, save_mcmc = FALSE,
                   M = 5e2, nburnin = 2e2
  )
  res.q$plot_infection
