...
# pi(t) (time unit can be changed from day to week or any numbers of days)
#
...
#' @param R the time series of daily observed removed compartment proportions,
#' including death and recovered.
#' @param pi0 the time-dependent transmission rate modifier \eqn{\pi(t)} between
#'  0 and 1.
#' @param change_time the change points over time for step function pi, defalt
#' value is \code{NULL}.
#' @param exponential logical, whether \eqn{\pi(t)} is exponential
#' \eqn{\exp(-\lambda_0t)} or not; the default is \code{FALSE}.
#' @param lambda0 the rate of decline in the exponential survival function in
#' \eqn{\exp(-\lambda_0t)}.
#' @param begin_str the character of starting time, the default is "01/13/2020".
#' @param T_fin the end of follow-up time after the beginning date
#' \code{begin_str}, the default is 200. This value must be longer than the
#' length of your input observed time series data.
#' @param nchain the number of MCMC chains generated by
#' \code{\link[rjags]{rjags}}, the default is 4.
#' @param nadapt the iteration number of adaptation in the MCMC. We recommend
#' using at least the default value 1e4 to obtained fully adapted chains.
#' @param M the number of draws in each chain, with no thinning. The default is
#' M=5e2 but suggest using 5e5.
#' @param thn the thinning interval between mixing. The total number of draws
...
#' @param dic logical, whether compute the DIC (deviance information criterion)
#' for model selection.
#' @param death_in_R the numeric value of average of cumulative deaths in the
#' removed compartments. The default is 0.4 within Hubei and 0.02 outside Hubei.
#' @param casename the string of the job's name. The default is "tvt.eSIR2".
#' @param beta0 the hyperparameter of average transmission rate, the default is
#' the one estimated from the SARS first-month outbreak (0.2586).
#' @param gamma0 the hyperparameter of average removed rate, the default is the
#' one estimated from the SARS first-month outbreak (0.0821).
#' @param R0 the hyperparameter of the mean reproduction number R0. The default
#' is thus the ratio of \code{beta0/gamma0}, which can be specified directly.
#' @param gamma0_sd the standard deviation for the prior distrbution of the
#' removed rate \eqn{\gamma}, the default is 0.1.
#' @param R0_sd the standard deviation for the prior disbution of R0, the
#' default is 1.
#' @param file_add the string to denote the location of saving output files and
#' tables.
#' @param save_files logical, whether save (\code{TRUE}) results or not
#' (\code{FALSE}). This enables to save summary tables, trace plots, and plots
#' of the posterior means of the first-order derivatives of the infection
#' prevalence process \eqn{\theta_t^I}.
#' @param save_mcmc logical, whether save (\code{TRUE}) all the MCMC outputs or
...
#' @param add_death logical, whether add the approximate death curve to the
#' plot, default is false.
#' @param eps a non-zero controller so that all the input \code{Y} and \code{R}
#' values would be bounded above 0 (at least \code{eps}). Its default value is
#' 1e-10
#' @param time_unit numeric, newly added argument, which can be changed to an
...
#' credible interval and median.}
#' \item{dic_val}{the output of \code{dic.samples()} in  \code{\link[rjags]{dic.samples}},
#' computing deviance information criterion for model comparison.}
...
tvt.eSIR2 <- function(Y,
  ...
  if(time_unit <= 0) warning("time_unit cannot be zero or negative." ,
                             " Set it to the default value 1. ")
  if(length(time_unit) > 1) stop("time_unit must be a numeric value",
                                 " (not a vector or list).")
  time_unit <- max(ceiling(time_unit), 1)
  T_prime <- length(Y)
  ...
  if (exponential == FALSE & !is.null(change_time) & !is.null(pi0)) {
    ...
    change_time_chorn <- c(
      begin - time_unit,
      chron(dates. = change_time),
      end
    )
    pi <- rep(
      pi0,
      round(diff(change_time_chorn) / time_unit)
    )
...
")
  ...
  thetaI_band <- data.frame(
    t(apply(theta_p[, -1, 2], # delete the first initial column
            2,
            quantile,
            probs = c(0.025, 0.975),
            na.rm = TRUE
  ...
  colnames(sample_dthetaI_mat) <- c(as.character(chron_ls))
  ...
  dthetaI_mean_data <- data.frame(dthetaI, date = chron_ls)
  ...
  spaghetti_plot <- ggplot() +
    ...
    scale_x_continuous(
      labels = as.character(chron_ls)[
        seq(1, T_fin, length.out = 10)
      ],
      breaks = as.numeric(chron_ls[
        seq(1, T_fin, length.out = 10)
      ]
      )
    ) +
    annotate(
      ...
      x = as.numeric(chron_ls[T_prime]) + 12/time_unit,
      y = spaghetti_ht,
      color = "blue"
    ) +
    annotate(
      ...
      x = as.numeric(dthetaI_tp1_date) + 12/time_unit,
      y = spaghetti_ht * 1.25,
      color = "darkgreen"
    ) +
    geom_vline(
  ...
  plot1 <- ggplot(
    ...
    scale_x_continuous(
      ...
      breaks = seq(1, T_fin, length.out = 10)
    ) +
    scale_fill_discrete(
      name = "Posterior",
      labels = c(
  ...
  if (dthetaI_tp2 > dthetaI_tp1) {
    plot1 <- plot1 + geom_vline(
      ...
      annotate(
        geom = "text",
        label = as.character(chron(dthetaI_tp2_date, format = "mon day")),
        x = dthetaI_tp2 + 12/time_unit, y = y_text_ht * 1.5, color = "purple"
      )
  }
  ...
  data_comp_R <- data.frame(
    time = 1:T_fin,
    rbind(thetaR_band, R_band),
    phase = c(rep("pre", nrow(thetaR_band)),
              rep("post", nrow(R_band))),
    mean = thetaR_mean,
    median = thetaR_med,
  ...
  data_poly_R <- data.frame(
    y = c(thetaR_band$upper,
  ...
  if (save_plot_data) {
    other_plot <- list(T_prime = T_prime,
                       T_fin = T_fin,
                       chron_ls = chron_ls,
    ...
    infection_plot_ls <- list(y_text_ht = y_text_ht,
                              data_poly = data_poly,
                              data_comp = data_comp,
                              data_pre = data_pre)
    removed_plot_ls <- list(r_text_ht = r_text_ht,
                            data_poly_R = data_poly_R,
                            data_comp_R = data_comp_R,
                            data_pre_R = data_pre_R)
    plot_data_ls <- list(casename = casename,
  ...
  gelman_out_name <- c("R0",
                       ...
                       "lambdaR",
                       "k")
  gelman_diag_list <- lapply(gelman_out_name,
                             function(name) {
                               if (name == "theta") {
                                 tryCatch(
                                   gelman.diag(
